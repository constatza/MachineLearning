namespace MGroup.MachineLearning.TensorFlow
{
	...
	public class Encoder1D : ISurrogateModel2DTo2D
	{
		...
		public Encoder1D(int latentSpaceSize, int caeBatchSize = 10, int caeNumEpochs = 40, float caeLearningRate = 5E-4f,
		...
		{
			//double[,,] trainX = trainSolutions.AddEmptyDimensions(false, false, true);
			double[,,] trainX = trainSolutions.AddEmptyDimensions(false, true, false);
		...
		{
			//double[,,] testSolutionsWithChannels = testSolutions.AddEmptyDimensions(false, false, true);
			double[,,] testSolutionsWithChannels = testSolutions.AddEmptyDimensions(false, false, true);

			var watch = new Stopwatch();
			StreamWriter writer = _initOutputStream();

			writer.WriteLine("Testing Encoder1D:");
			watch.Start();
			double[,] predictions = _encoder.EvaluateResponses(testSolutionsWithChannels);
