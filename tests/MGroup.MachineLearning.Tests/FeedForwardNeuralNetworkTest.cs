...
namespace MGroup.MachineLearning.Tests
{
    public class FeedForwardNeuralNetworkTest
    {
        ...
        private static void CheckPredictionAccuracy(double[,] data,double[,] prediction)
        {
            ...
            norm = Math.Sqrt(norm);
            Assert.True(norm < 0.03, $"Prediction norm was above threshold (norm value: {norm})");
        }

        private static void CheckGradientAccuracy(double[][,] dataGradient, double[][,] gradient)
        {
